...
/**
 * Parses the currency string and returns the number of cents.
 *
...
export function cents(localeString: string, thousandsSeparator: string = ',', decimalPoint: string = '.'): number {
  let value = localeString.replace(
    thousandsSeparator === ',' ? commaRegex : new RegExp(thousandsSeparator, 'g'), ''
  );
  ...
  return parseInt(value[0] === '$' ? value.slice(1) : value);
}
